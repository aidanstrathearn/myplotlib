...
impl Figure {
    ...
    fn from_plotter(plotter: Plotter) -> Self {
        Self {
            rows: 1,
            columns: 1,
            axes: vec![plotter],
...
fn draw_axes(ui: &mut egui::Ui, plotter: &Plotter, axes_index: usize) -> egui::Rect {
    ...
    let response = plot.show(ui, |plot_ui| {
        ...
        let colour_offset = plotter.series.len();

        for (index, line) in plotter.horizontal_lines.iter().enumerate() {
        ...
        let colour_offset = colour_offset + plotter.horizontal_lines.len();

        for (index, line) in plotter.vertical_lines.iter().enumerate() {
            let colour = MATPLOTLIB_COLORS[(colour_offset + index) % MATPLOTLIB_COLORS.len()];
            let legend_name = line.label.as_deref().unwrap_or_default();
